...
export class DefaultSdkService implements SdkService {
   ...
   */
  async unlock(userId: UserId, request: InitUserCryptoRequest): Promise<UserKey | null> {
    if (!(await this.longLivedEnabled(userId))) {
      return null;
    }
    ...
    // Supplied here rather than by the caller: the token is owned by this service (it is the one unlock
    // input the pushing owners don't hold), and the legacy path passes it too, so leaving it to callers
    // would silently skip the V2 upgrade on whichever path forgot it.
    const v2UpgradeToken = await firstValueFrom(
      this.v2UpgradeTokenStateService.v2UpgradeToken$(userId),
    );
    ...
    await this.withClient(userId, async (client) => {
      ...
      await client
        .crypto()
        .initialize_user_crypto({ ...request, upgradeToken: v2UpgradeToken ?? undefined });
  ...
  async setOrgKeys(userId: UserId, orgKeys: Record<OrganizationId, EncString>): Promise<void> {
    ...
    await this.withClient(userId, (client) =>
      client.crypto().initialize_org_crypto(toSdkOrgCrypto(orgKeys)),
    );
