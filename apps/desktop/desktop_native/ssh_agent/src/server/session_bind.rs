...
/// Parses and verifies a `session-bind@openssh.com` extension payload (bytes after the extension
/// name string).
///
/// # Returns
/// `Some(is_forwarding)` on success, `None` on any parse or verification failure.
//
// NOTE: currently we only utilize the is_forwarding property. If we expand this in the future,
// a dedicated struct to contain the session state can be returned.
pub(super) fn parse_and_verify(payload: &[u8]) -> Option<bool> {
...
mod tests {
    ...
    #[test]
    fn valid_ed25519_not_forwarding_returns_some_false() {
        let keypair = Ed25519Keypair::random(&mut OsRng);
        let payload = make_session_bind_payload_ed25519(&keypair, &[0x42u8; 32], false);
        assert_eq!(parse_and_verify(&payload), Some(false));
    }

    #[test]
    fn valid_ed25519_forwarding_returns_some_true() {
        let keypair = Ed25519Keypair::random(&mut OsRng);
        let payload = make_session_bind_payload_ed25519(&keypair, &[0x42u8; 32], true);
        assert_eq!(parse_and_verify(&payload), Some(true));
    }

    #[test]
    fn valid_rsa_sha256_returns_some_false() {
    ...
    #[test]
    fn valid_rsa_sha512_returns_some_false() {
        let keypair = RsaKeypair::random(&mut OsRng, 2048).unwrap();
    ...
    #[test]
    fn tampered_signature_bytes_returns_none() {
        let keypair = Ed25519Keypair::random(&mut OsRng);
        let mut payload = make_session_bind_payload_ed25519(&keypair, &[0x42u8; 32], false);
        // Flip second-to-last byte — within sig bytes, before the is_forwarding flag
        let n = payload.len();
        payload[n - 2] ^= 0xFF;
        assert!(parse_and_verify(&payload).is_none());
    }

    #[test]
    fn wrong_key_in_hostkey_returns_none() {
        let keypair_a = Ed25519Keypair::random(&mut OsRng);
        let keypair_b = Ed25519Keypair::random(&mut OsRng);

        let session_id = [0x42u8; 32];
        // hostkey = keypair_a's public key, but signature is from keypair_b
        let private_a =
    ...
    #[test]
    fn rsa_sha1_algorithm_returns_none() {
        let keypair = RsaKeypair::random(&mut OsRng, 2048).unwrap();
    ...
    #[test]
    fn rsa_unsupported_algorithm_returns_none() {
        let keypair = RsaKeypair::random(&mut OsRng, 2048).unwrap();
    ...
    fn empty_payload_returns_none() {
        assert!(parse_and_verify(&[]).is_none());
    }

    #[test]
    fn truncated_after_hostkey_returns_none() {
        let keypair = Ed25519Keypair::random(&mut OsRng);
        let private_key =
        ...
        let mut payload = Vec::new();
        write_ssh_string(&mut payload, &hostkey_bytes);
        // No session_id follows

        assert!(parse_and_verify(&payload).is_none());
    }

    #[test]
    fn truncated_after_session_id_returns_none() {
        let keypair = Ed25519Keypair::random(&mut OsRng);
        let private_key =
            ssh_key::PrivateKey::new(KeypairData::Ed25519(keypair.clone()), "").unwrap();
        let hostkey_bytes = private_key.public_key().to_bytes().unwrap();
        ...
        // No sig_outer follows

        assert!(parse_and_verify(&payload).is_none());
    }

    #[test]
    fn truncated_sig_outer_returns_none() {
        let keypair = Ed25519Keypair::random(&mut OsRng);
        let private_key =
            ssh_key::PrivateKey::new(KeypairData::Ed25519(keypair.clone()), "").unwrap();
        let hostkey_bytes = private_key.public_key().to_bytes().unwrap();
        ...
        write_ssh_string(&mut payload, &hostkey_bytes);
        write_ssh_string(&mut payload, &[0x42u8; 32]);
        // sig_outer claims 100 bytes but only 5 follow
    ...
    #[test]
    fn invalid_utf8_alg_name_returns_none() {
        let keypair = Ed25519Keypair::random(&mut OsRng);
        let private_key =
            ssh_key::PrivateKey::new(KeypairData::Ed25519(keypair.clone()), "").unwrap();
        let hostkey_bytes = private_key.public_key().to_bytes().unwrap();
        ...
        let mut sig_outer = Vec::new();
        write_ssh_string(&mut sig_outer, &[0xFFu8, 0xFE]); // invalid UTF-8
    ...
    fn non_one_byte_treated_as_not_forwarding() {
        let keypair = Ed25519Keypair::random(&mut OsRng);
        let mut payload = make_session_bind_payload_ed25519(&keypair, &[0x42u8; 32], false);
        *payload.last_mut().unwrap() = 2u8; // 0x02 is not 0x01, so not forwarding
        assert_eq!(parse_and_verify(&payload), Some(false));
    }
    ...
    fn missing_forwarding_byte_defaults_to_false() {
        let keypair = Ed25519Keypair::random(&mut OsRng);
        let mut payload = make_session_bind_payload_ed25519(&keypair, &[0x42u8; 32], false);
        payload.pop(); // remove the is_forwarding byte
        assert_eq!(parse_and_verify(&payload), Some(false));
    }
}
