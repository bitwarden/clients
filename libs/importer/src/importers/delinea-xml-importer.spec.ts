...
describe("Delinea Xml Importer", () => {
  ...
  it("should import Secrets without a url slug as Secure Notes with custom fields for all Secret info except notes", async () => {
    ...
    expect(result.ciphers[0].notes).toEqual("Level 1 = 1024");
  ...
  it("should import Secrets with a url slug as Logins with Secret info set in the correct place", async () => {
    ...
    expect(result.ciphers.length).toEqual(1);
    expect(result.ciphers[0].type).toEqual(CipherType.Login);
    expect(result.ciphers[0].login.username).toEqual("myUser");
    expect(result.ciphers[0].login.uris.length).toEqual(1);
    expect(result.ciphers[0].login.uris[0].uri).toEqual("https://bitwarden.com");
    expect(result.ciphers[0].login.password).toEqual("SoftBatchCookies123!");
