...
pub async fn decrypt_with_admin_and_service(
    admin_exe: &str,
    service_exe: &str,
    encrypted: &str,
) -> Result<String> {
    ...
    debug!("Launching '{}' as admin...", service_exe);
    decrypt_with_admin(admin_exe, service_exe, encrypted);
...
fn decrypt_with_admin(admin_exe: &str, service_exe: &str, encrypted: &str) {
    ...
    let runas_wide = OsStr::new("runas")
        ...
        .chain(std::iter::once(0))
        .collect::<Vec<u16>>();
    let parameters = OsStr::new(&format!(
        r#"--service-exe "{}" --encrypted "{}""#,
        service_exe, encrypted
    ))
    .encode_wide()
    .chain(std::iter::once(0))
    .collect::<Vec<u16>>();
