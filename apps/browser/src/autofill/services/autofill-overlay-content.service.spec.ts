...
describe("AutofillOverlayContentService", () => {
  ...
  describe("getSameOriginIframeOffset", () => {
    afterEach(() => {
      document.body.innerHTML = "";
    });

    it("returns null for an element in the top-level document", () => {
      const input = document.createElement("input");
      document.body.appendChild(input);

      const result = autofillOverlayContentService["getSameOriginIframeOffset"](input);

      expect(result).toBeNull();
    });

    it("returns the iframe's bounding rect offset for an element inside a same-origin iframe", () => {
    ...
    it("accumulates offsets across nested same-origin iframes", () => {
      ...
      const input = innerIframe.contentDocument!.createElement("input");
      innerIframe.contentDocument!.body.appendChild(input);
      jest.spyOn(outerIframe, "getBoundingClientRect").mockReturnValue({
        top: 100,
        left: 50,
        width: 800,
        height: 400,
      ...
      jest.spyOn(innerIframe, "getBoundingClientRect").mockReturnValue({
        top: 20,
        left: 10,
        width: 400,
        height: 200,
  ...
  describe("getMostRecentlyFocusedFieldRects", () => {
    afterEach(() => {
      document.body.innerHTML = "";
    });

    it("returns the element's bounding rect unchanged for a top-level element", async () => {
      const input = document.createElement("input") as ElementWithOpId<HTMLInputElement>;
      document.body.appendChild(input);
      jest
        .spyOn(
          autofillOverlayContentService as any,
          "getBoundingClientRectFromIntersectionObserver",
        )
      ...
      const result = await autofillOverlayContentService["getMostRecentlyFocusedFieldRects"](input);

      expect(result).toMatchObject({ top: 50, left: 100, width: 200, height: 30 });
    });

    it("adds the iframe offset to the element rect for an element inside a same-origin iframe", async () => {
      const iframe = document.createElement("iframe");
      document.body.appendChild(iframe);
      const input = iframe.contentDocument!.createElement(
        "input",
      ) as ElementWithOpId<HTMLInputElement>;
      iframe.contentDocument!.body.appendChild(input);
      jest
      ...
      expect(result).toMatchObject({ top: 110, left: 70, width: 200, height: 30 });
    });

    it("applies the iframe offset to the IntersectionObserver rect when available", async () => {
      const iframe = document.createElement("iframe");
      document.body.appendChild(iframe);
      const input = iframe.contentDocument!.createElement(
        "input",
      ...
      jest
        ...
        .mockResolvedValue({ top: 15, left: 25, width: 200, height: 30 } as DOMRectReadOnly);
      jest.spyOn(iframe, "getBoundingClientRect").mockReturnValue({
        top: 100,
        left: 50,
        width: 800,
      ...
      expect(result).toMatchObject({ top: 115, left: 75, width: 200, height: 30 });
    });
  });
});
