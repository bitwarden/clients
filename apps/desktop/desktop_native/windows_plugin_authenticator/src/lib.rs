...
pub fn register() -> Result<(), String> {
    ...
    let config = match read_plugin_config_file() {
        Ok(config) => config,
        Err(err) => {
            // if this errors on reading the plugin file, this is probably not an Appx, so just log
            // and return.
            tracing::debug!("Could not read the plugin authenticator config file on registration, assuming this is not an Appx. Skipping plugin registration: {err:#}");
            return Ok(());
        }
    };
