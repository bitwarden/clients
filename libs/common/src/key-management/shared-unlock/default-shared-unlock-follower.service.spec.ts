...
import { DefaultSharedUnlockFollowerService } from "./default-shared-unlock-follower.service";
import { SharedUnlockSettingsService } from "./shared-unlock-settings.service";

describe("DefaultSharedUnlockFollowerService", () => {
  ...
  let follower: { start: jest.Mock; handle_device_event: jest.Mock };

  let sut: DefaultSharedUnlockFollowerService;
  ...
  beforeEach(async () => {
    ...
    jest.spyOn(SharedUnlockFollower, "try_new").mockReturnValue(follower as never);
    ...
    sut = new DefaultSharedUnlockFollowerService(
      ipcService,
      accountService,
      lockService,
      keyService,
  ...
  describe("on lock", () => {
    it.each([LockSource.Manual, LockSource.VaultTimeout])(
      "reports a %s lock to the follower",
      async (source) => {
        await sut.start();

        await onLockAction()(userId, source);

        expect(follower.handle_device_event).toHaveBeenCalledWith({
    ...
    it("ignores a lock caused by shared unlock", async () => {
      ...
      expect(follower.handle_device_event).not.toHaveBeenCalled();
    });
  });

  describe("on unlock", () => {
    it.each([UnlockSource.Manual, UnlockSource.NeverLock])(
      "reports a %s unlock to the follower",
      async (source) => {
        await sut.start();
