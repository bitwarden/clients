...
otherwise render the logo twice. Content that still belongs in the title bar goes in `start`,
...
it. Unlike `start` and `title-end`, this slot is not flag-gated — it renders in the one-bar header
too.
