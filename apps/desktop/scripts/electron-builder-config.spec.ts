...
function configFor(line: string): BuildConfig {
  const raw = parseConfigureArgs(args(line));
  expect(validate(raw)).toEqual([]);
  return toBuildConfig(raw);
}

/// Platform baselines without a distribution channel, which every case adds for itself. The
/// macOS certificate is required for any macOS build; a case naming its own later overrides it.
const MAC =
  "--build-dir build-mac --architecture universal " +
...
describe("electronBuilderTargets", () => {
  it("maps distribution channels to electron-builder targets", () => {
    const config = configFor(`${MAC} --distribution-channel dmg --distribution-channel mac-zip`);
...
describe("the beta channel", () => {
  const betaOf = (line: string) => applyBuildConfig(base, configFor(`${line} --channel beta`));
  const stableOf = (line: string) => applyBuildConfig(base, configFor(line));
