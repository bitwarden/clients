...
describe("Fido2VaultComponent", () => {
  ...
  beforeEach(async () => {
    ...
    // Setup default resolved promise for cipher service
  ...
  describe("ngOnInit", () => {
    ...
    it("should handle error when no active session found", async () => {
      ...
      // This will cause an error when trying to access availableCipherIds$
      await expect(component.ngOnInit()).rejects.toThrow();
    });

    it("should handle cipher loading error gracefully", async () => {
      const error = new Error("Failed to load ciphers");
      mockCipherService.getAllDecryptedForIds.mockRejectedValue(error);
    ...
    it("should handle missing active user", async () => {
      ...
      // Component will still initialize, but cipher service will be called with undefined userId
      expect(mockCipherService.getAllDecryptedForIds).toHaveBeenCalledWith(
        undefined,
        expect.any(Array),
      );
    });
    ...
    it("should filter out deleted ciphers", async () => {
      const mockCiphers = createMockCiphers();
      mockCiphers[1].deletedDate = new Date(); // Mark second cipher as deleted
      mockCipherService.getAllDecryptedForIds.mockResolvedValue(mockCiphers);
      ...
      // Check the ciphers were filtered
  ...
  describe("ngOnDestroy", () => {
    it("should restore header visibility and clean up", async () => {
      // Initialize the component first to create the cipherIdsSubject
      await component.ngOnInit();
  ...
  describe("chooseCipher", () => {
    ...
    it("should not choose cipher when contains excluded ciphers", async () => {
      const cipher = createMockCiphers()[0];
      component.containsExcludedCiphers = true;
    ...
    it("should handle no active session", async () => {
      ...
      // The component will still navigate and reset modal even without a session
      expect(mockRouter.navigate).toHaveBeenCalledWith(["/"]);
      expect(mockDesktopSettingsService.setModalMode).toHaveBeenCalledWith(false);
    });
  });
  ...
  describe("loadCiphers", () => {
    it("should handle when session has no availableCipherIds$", async () => {
      mockSession.availableCipherIds$ = undefined;

      // This will throw an error when trying to pipe on undefined
      await expect(component.ngOnInit()).rejects.toThrow();
    });

    it("should handle cipher service errors", async () => {
      const error = new Error("Cipher service error");
      mockCipherService.getAllDecryptedForIds.mockRejectedValue(error);
  ...
  describe("ciphers$ observable", () => {
    it("should emit filtered ciphers when cipher IDs change", async () => {
      const mockCiphers = createMockCiphers();
      const cipherIdsSubject = new BehaviorSubject(mockCipherIds);
      mockSession.availableCipherIds$ = cipherIdsSubject.asObservable();
      mockCipherService.getAllDecryptedForIds.mockResolvedValue(mockCiphers);

      await component.ngOnInit();

      const newCipherIds = ["cipher-1"];
      cipherIdsSubject.next(newCipherIds);
