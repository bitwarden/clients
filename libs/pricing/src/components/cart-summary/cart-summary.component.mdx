...
- [Examples](#examples)
  ...
  - [All Proration Invoice](#all-proration-invoice)
  - [Recurring Items With Proration](#recurring-items-with-proration)
...
| -------------------- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------- |
| `cart`               | `Cart`                   | **Required.** The cart data containing all products, discounts, tax, and cadence                                              |
...
export type CartItem = {
  ...
  discounts?: CartDiscount[]; // Optional item-level discounts, rendered as indented rows beneath the line
  hideBreakdown?: boolean; // Optional: hide cost breakdown (quantity × unit price)
};

export type Cart = {
  passwordManager: {
    seats?: CartItem; // Absent on an all-proration transition invoice
    additionalStorage?: CartItem; // Optional additional storage
    prorationCharges?: CartItem[]; // Optional mid-cycle proration charges, rendered as their own lines
  };
  secretsManager?: {
    // Optional SM section
    seats?: CartItem; // SM seats
    additionalServiceAccounts?: CartItem; // Optional service accounts
    prorationCharges?: CartItem[]; // Optional mid-cycle proration charges
  };
  cadence: "annually" | "monthly"; // Billing period for entire cart
  hidePricingTerm?: boolean; // Optional set if term should be removed
  discounts?: CartDiscount[]; // Optional cart-level discounts, rendered in the grouped summary block (Subtotal → discounts → Estimated tax)
  credit?: Credit; // Optional account credit
  estimatedTax: number; // Tax amount
  total?: number; // Optional authoritative total; when absent the component computes it
};
...
// The renderer's discount shape. Legacy carts supply only `type` and `value`, and the component
// derives the applied amount and display label from them. Preview-driven carts (built by the
// invoice-preview adapter from the server's `InvoicePreviewDiscount`) also supply `amount` — the
// authoritative applied amount computed by Stripe — and `label` (the coupon name), which the
// component prefers over derivation.
export type CartDiscount = {
  type: DiscountType; // DiscountTypes.AmountOff | DiscountTypes.PercentOff
  value: number; // Dollar amount or percentage (20 for 20%)
  amount?: number; // Optional authoritative applied amount in dollars
  label?: string; // Optional coupon name, preferred over the derived label
  durationInMonths?: number; // Optional repeating-coupon duration, appended to the derived label
};
```
...
### All Proration Invoice

Show a seatless mid-cycle transition invoice — only one-time proration charges and a credit, no
recurring line items. The total is a one-time amount, so the cart sets `hidePricingTerm` and the
term is dropped:

<Canvas of={CartSummaryStories.AllProrationInvoice} />
...
### Recurring Items With Proration

Show a renewal with a mid-cycle change: recurring seat and storage lines alongside their own
proration charge rows, with the billing term retained:

<Canvas of={CartSummaryStories.RecurringWithProration} />
...
<Canvas of={CartSummaryStories.SinglePerLineDiscount} />
...
renders indented under its line item, while the cart-level discount renders in a grouped summary
block between two dividers: Subtotal → cart-level discount(s) → Estimated tax, followed by the
Total. The Subtotal row (extended prices net of any per-line discounts, matching Stripe's invoice
subtotal) appears only when a cart-level discount is present:

<Canvas of={CartSummaryStories.PerLineAndCartWideStacked} />
...
- **Discount Support**: Displays both cart-level and item-level discounts. Cart-level discounts
  appear in a dedicated section, while each line item's `discounts` render as indented rows under
  the line item they apply to. Supports both percentage-based and fixed-amount discounts with green
  success styling
...
- Omit required Cart fields (cadence, estimatedTax)
- Invent a proration charge without a purchasable reference — the label resolves from the reference
  and falls back to the product group
