...
/// The Appx extensions are generated per channel from one template, for the same reason the
/// entitlements are: the channel-specific parts are a name and a filename, not a second file.
export const APPX_DIR = "intermediates/appx";
export const APPX_EXTENSIONS_FILE = "custom-appx-extensions.xml";
export const APPX_EXTENSIONS_TEMPLATE = "custom-appx-extensions.xml";

/// Where each channel's plugin authenticator declares the COM class ID it serves. The app reads
/// it at runtime and the Appx manifest declares it, so both have to name the same file.
export const PLUGIN_AUTHENTICATOR_CONFIGS: Record<Channel, string> = {
  stable: "windows_plugin_authenticator_config.json",
  beta: "windows_plugin_authenticator_config.beta.json",
};
...
export interface WindowsDerived {
  /// Generated by configure. Absent unless this build declares Appx extensions at all, which
  /// only the channels shipping the passkey provider do.
  appxExtensions?: string;
}
...
export interface BuildConfig {
  ...
  windows?: {
    /// Whether the Appx has to end up signed. The certificate itself is not here: it is a
    /// secret, this file is written to disk and carried between machines, and CI only has the
    /// certificate at the moment it signs -- long after this was written.
    signAppx: boolean;
  };
...
export function validate(raw: RawOptions): string[] {
  ...
  if (raw.signAppx && !distributionChannels.includes("windows-appx")) {
    errors.push(
      "--sign-appx only applies to --distribution-channel windows-appx; the Store package is " +
        "submitted unsigned.",
    );
  }
...
export function toBuildConfig(raw: RawOptions, resolved: ResolvedInputs = {}): BuildConfig {
  ...
  return {
    ...
    ...(raw.signAppx ? { windows: { signAppx: true } } : {}),
    ...
    derived: {
      ...
      ...(platform === "windows" && channel === "beta"
        ? { windows: { appxExtensions: join(buildDir, APPX_DIR, APPX_EXTENSIONS_FILE) } }
        : {}),
      ...(platform === "macos"
        ? {
            macos: macosDerived(
