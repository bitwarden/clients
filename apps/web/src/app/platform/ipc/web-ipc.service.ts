...
// Web's only leader is the browser extension background. The SDK pings it (over the raw transport,
// bypassing crypto) to track reachability and gate handshakes; see the Rust `ReachabilityTracker`.
  ...
  override async init() {
    try {
      ...
      window.addEventListener("message", async (event: MessageEvent) => {
        ...
        // Reachability pings/pongs ride this same channel as ordinary IPC frames (distinguished by
        // topic); the SDK records liveness and answers pings itself.
      ...
      await super.initWithClient(
        IpcClient.newWithSdkInMemorySessions(this.communicationBackend, { pingTargets: [LEADER] }),
      );
