...
describe("GovernanceDashboardComponent", () => {
  ...
  describe("policy rendering", () => {
    it("renders pamPolicyHumanApproval for a human_approval policy", async () => {
      await setup({
        summary: makeSummary([
          {
            CollectionId: "col-1",
      ...
      expect(i18nService.t).toHaveBeenCalledWith("pamPolicyHumanApproval");
    });

    it("renders pamPolicyIpAllowlist with cidr count for an ip_allowlist policy", async () => {
      await setup({
        summary: makeSummary([
          {
            CollectionId: "col-1",
            CollectionName: "Servers",
            Policy: { Kind: "ip_allowlist", Cidrs: ["10.0.0.0/8", "192.168.0.0/16"] },
          },
        ]),
      });

      expect(i18nService.t).toHaveBeenCalledWith("pamPolicyIpAllowlist", "2");
    });

    it("renders all_of by joining each child summary with pamPolicySeparator", async () => {
      await setup({
        summary: makeSummary([
          {
            CollectionId: "col-1",
      ...
      expect(i18nService.t).toHaveBeenCalledWith("pamPolicySeparator");
      expect(i18nService.t).toHaveBeenCalledWith("pamPolicyHumanApproval");
      expect(i18nService.t).toHaveBeenCalledWith("pamPolicyIpAllowlist", "1");
    });
  });
  ...
  describe("click-throughs", () => {
    it("renders links that preserve the collectionId query param", async () => {
      ...
      const requireLeaseLink: HTMLAnchorElement = fixture.nativeElement.querySelector(
        "[data-testid='link-require-lease']",
      );
      ...
      expect(requireLeaseLink.textContent?.trim()).toBe("4");
      ...
      expect(requireLeaseLink.getAttribute("href")).toContain(
  ...
  describe("last activity", () => {
    ...
    it("uses the row CollectionGovernanceRowResponse object directly when summaryOverride is supplied", async () => {
      ...
      expect(pamApiService.getGovernanceSummary).not.toHaveBeenCalled();
      const requireLeaseLink = fixture.nativeElement.querySelector(
        "[data-testid='link-require-lease']",
      );
      expect(requireLeaseLink.textContent?.trim()).toBe("1");
    });
  });
