...
describe("RotationScheduleInputComponent", () => {
  ...
  it("renders the custom cron error on the field the operator typed in", async () => {
    presetCtrl().setValue(QuartzSchedulePreset.Custom);
    customCtrl().setValue("not-a-cron");
    await fixture.whenStable();
    fixture.detectChanges();

    const custom: HTMLInputElement = fixture.nativeElement.querySelector(
      "#rotation-schedule-input_input_custom-cron",
    );
    custom.dispatchEvent(new Event("blur"));
    fixture.detectChanges();

    const error = custom.closest("bit-form-field")?.querySelector("bit-error");
    expect(error).not.toBeNull();
    expect(error!.textContent).toContain("pamRotationScheduleInvalidCron");
  });

  it("clears the custom cron error once the expression is Quartz-shaped", async () => {
    presetCtrl().setValue(QuartzSchedulePreset.Custom);
    customCtrl().setValue("not-a-cron");
    await fixture.whenStable();
    fixture.detectChanges();
    ...
    expect(custom.closest("bit-form-field")?.querySelector("bit-error")).toBeNull();
  });

  it("leaves an empty custom field unflagged, since that is no schedule", async () => {
    presetCtrl().setValue(QuartzSchedulePreset.Custom);
    customCtrl().setValue("");
    await fixture.whenStable();
    fixture.detectChanges();
