...
Pass in `null` to not display a title. If no title is provided, a default title will be used if callout type is `warning | danger`.

<Canvas of={stories.NoTitle} />

## Close Button

A close button can be added by setting `persistent` to `false` and attaching an `(onDismiss)` handler.
