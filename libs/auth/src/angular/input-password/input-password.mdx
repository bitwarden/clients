...
- Emergency Access Takeover<br /><br />

**`AccountRecovery`**

Used when an org admin resets a member's master password via the Account Recovery dialog. Like
`ChangePasswordDelegation`, no `userId` or `email` is passed down and the dialog owns the
Save/Cancel buttons. Unlike `ChangePasswordDelegation`, there is only **one** password field (no
confirm field) and the label reads "Temporary master password". When a policy is in effect, the
requirements are listed inline beneath the field rather than in the banner callout.<br /><br />
...
- The new password and confirmed new password are the same (all flows except `AccountRecovery`,
  which has no confirm field)
...
Some of our set/change password flows use dialogs, such as Emergency Access Takeover
(`ChangePasswordDelegation`) and Account Recovery (`AccountRecovery`). Because dialogs have their
own buttons, we don't want to display an additional Submit button in the `InputPasswordComponent`
when embedded in a dialog.
