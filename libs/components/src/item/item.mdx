...
| Size             | Vertical padding | Trailing padding |
| ---------------- | ---------------- | ---------------- |
| `base` (default) | 8px              | 12px             |
| `lg`             | 12px             | 16px             |
...
| Content               | Spacing from its neighbors |
| --------------------- | -------------------------- |
| Plain text            | 8px                        |
| Link                  | 8px                        |
| Badge / chip          | 4px                        |
...
The space between two neighbors is the larger of their two values, not the sum — a badge beside
plain text gets 8px, while a badge beside an icon button gets 4px. These values apply between
children only; the item's trailing padding owns the space after the last child, so the last child
never adds its own. Wrapped content is measured by what it contains, no matter how many components
deep, so an icon button inside `<bit-item-action>` or inside an app-level component is still treated
as an icon button. Spacing a wrapper applies to its own contents is ignored inside the slot.
