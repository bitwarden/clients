...
- [Examples](#examples)
  ...
  - [Past Due Without Details](#past-due-without-details)
  - [Unpaid](#unpaid)
  - [Unpaid Without Suspension Date](#unpaid-without-suspension-date)
  - [Canceled](#canceled)
  - [Enterprise](#enterprise)
  - [Callout Suppressed](#callout-suppressed)
  - [Preview-Driven With Projected Warning](#preview-driven-with-projected-warning)
- [Features](#features)
...
| `subscription`      | `BitwardenSubscription \| SubscriptionPreview` | **Required.** The subscription data including status, cart, and storage (see Data Structure) |
| `showUpgradeButton` | `boolean`                                      | **Optional.** Whether to show the upgrade callout (default: `false`)                         |
| `hideCallout`       | `boolean`                                      | **Optional.** Hides the status callout entirely while the badge stays (default: `false`)     |
...
The component also accepts `SubscriptionPreview` — the preview-driven counterpart projected from a
Stripe invoice preview. It differs from `BitwardenSubscription` in two ways:

- `storage` is optional (the server returns none for subscribers without a storage allowance).
- `suspension` and `gracePeriod` are optional for `past_due`/`unpaid` — the server omits them when
  no open overdue invoice qualifies, and the card renders the past-due/paused fallback header
  instead of a suspension date.

## Content Projection

The card projects arbitrary content between its callout and the card edge via `<ng-content>`. Host
pages use this to place contextual warnings (e.g. the scheduled price-increase warning) inside the
card without the card knowing about them. See the "Preview-Driven - With Projected Warning" story.
...
**Actions available:** Keep Subscription

### Past Due Without Details

Preview-driven `past_due` subscription with no suspension date — the header falls back to the "will
pause soon" copy:

<Canvas of={SubscriptionCardStories.PastDueWithoutDetails} />
...
### Unpaid Without Suspension Date

Preview-driven `unpaid` subscription with no suspension date — the header falls back to the "paused"
copy:

<Canvas of={SubscriptionCardStories.UnpaidNoSuspensionDate} />
...
`hideCallout` removes the status callout entirely (e.g. reseller orgs exempt from billing
automation) while the badge and header stay:

<Canvas of={SubscriptionCardStories.CalloutSuppressed} />
...
### Preview-Driven With Projected Warning

Preview-driven subscription with contextual content projected into the card via `<ng-content>` (e.g.
the scheduled price-increase warning):

<Canvas of={SubscriptionCardStories.WithProjectedContent} />

```html
<billing-subscription-card [title]="'Teams Subscription'" [subscription]="previewSubscription">
  <app-organization-scheduled-price-increase-warning [organization]="organization">
...
- **Date Formatting**: Consistent date formatting throughout (MMMM d, y format)
...
- Provide complete `BitwardenSubscription` / `SubscriptionPreview` objects with all required fields
