...
**Note:** The `cart` object can also include:
- `hidePricingTerm` (boolean): Hides the billing term (e.g., "/ month", "/ year") from the header
- `hideBreakdown` (boolean) on individual `CartItem` objects: Hides the cost breakdown (quantity × unit price) for that specific line item
...
export type Cart = {
  ...
  hidePricingTerm?: boolean; // Optional: hide term (e.g., "/ month") in header
};
...
- **Hidden Pricing Term**: Cart can hide the billing term ("/ month" or "/ year") from the header
  using the `hidePricingTerm` property
...
- Use `hidePricingTerm` at the cart level when the billing term shouldn't be displayed in the header
