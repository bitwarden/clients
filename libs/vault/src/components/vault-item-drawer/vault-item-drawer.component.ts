...
export type VaultItemDrawerMode = "view" | "form";
...
export interface VaultItemDrawerParams {
   ...
   */
  mode: VaultItemDrawerMode;

  /**
   * The configuration object for the drawer and form.
   */
  formConfig: CipherFormConfig;

  /**
   * If true, the "edit" button will be disabled in the drawer.
   */
  disableForm?: boolean;
  ...
  /**
   * If true, the drawer is being opened from the admin console.
   */
...
export const VaultItemDrawerResult = {
  ...
  /**
   * The drawer was closed to navigate the user the premium upgrade page.
   */
...
export type VaultItemDrawerResult = UnionOfValues<typeof VaultItemDrawerResult>;

// FIXME(https://bitwarden.atlassian.net/browse/CL-764): Migrate to OnPush
// eslint-disable-next-line @angular-eslint/prefer-on-push-component-change-detection
@Component({
  selector: "vault-item-drawer",
...
export class VaultItemDrawerComponent implements OnInit, OnDestroy {
  /**
   * Reference to the drawer content element. Used to scroll to the top of the drawer when switching modes.
   * @protected
   */
  ...
  ngOnDestroy() {
    ...
    if (this._cipherModified) {
      this.dialogRef.close(VaultItemDrawerResult.Saved);
    }
  }
  ...
  restore = async () => {
    ...
    this.dialogRef.close(VaultItemDrawerResult.Restored);
  };
  ...
  cancel = async () => {
    ...
    if (this.params.mode === "view" || this.cipher == null || this.formConfig.mode === "clone") {
      this.dialogRef.close(this._cipherModified ? VaultItemDrawerResult.Saved : undefined);
      return;
    }
   ...
   * @private
   */
  private async changeMode(mode: VaultItemDrawerMode) {
