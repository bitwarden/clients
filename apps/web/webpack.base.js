...
module.exports.buildConfig = function buildConfig(params = {}) {
  ...
  console.log(`Building Web app${params.configName ? ` - ${params.configName} version` : ""}`);
  ...
  const plugins = [
    ...
    new AngularWebpackPlugin({
      tsconfig: params.tsConfig || "tsconfig.build.json",
      entryModule: params.main?.entryModule || "src/app/app.module#AppModule",
      sourceMap: true,
    }),
  ];
  ...
  const webpackConfig = {
    mode: NODE_ENV,
    devtool: "source-map",
    devServer: params.devServerConfig || defaultDevServer,
    target: "web",
    entry: {
      "app/polyfills": "./src/polyfills.ts",
      "app/main": params.main?.entry || "./src/main.ts",
