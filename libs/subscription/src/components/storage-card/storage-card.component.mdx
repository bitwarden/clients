...
- **`isNearlyFull()`**: Boolean indicating if storage usage is 95% or above (triggers red progress
  bar)
...
- **Progress bar**: Visual indicator of storage usage with dynamic color:
  - **Blue** (`tw-bg-primary-600`) for usage below 95%
  - **Red** (`tw-bg-danger-600`) for usage at or above 95% to warn users they're nearly full
...
Almost at capacity with **red progress bar** warning, user should consider adding storage:
...
Storage completely used with **red progress bar** indicating critical capacity:
...
Storage usage exceeds capacity with **red progress bar** (progress bar width capped at 100%):
...
{
  "storage": "Storage",
  "youHaveUsedStorage": "You have used {0} out of {1} GB of your encrypted file storage.",
  "addStorage": "Add storage",
  "removeStorage": "Remove storage"
}
```

The `youHaveUsedStorage` key accepts two parameters:

1. `{0}` - Used storage in GB
2. `{1}` - Total storage in GB
