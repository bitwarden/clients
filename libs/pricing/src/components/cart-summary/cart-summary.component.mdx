...
- [Examples](#examples)
  ...
  - [With Applied Account Balance](#with-applied-account-balance)
...
export type Cart = {
  ...
  credit?: Credit; // Optional pre-tax credit (e.g. prorated subscription credit)
  estimatedTax: number; // Tax amount
  accountCredit?: Credit; // Optional account balance applied to the taxed total; rendered between tax and total
...
### With Applied Account Balance

Show a cart where Stripe applied part of the customer's account balance to the invoice. The balance
is rendered as its own row between Estimated tax and Total so the rows still sum to the
authoritative total (the invoice's amount due):

<Canvas of={CartSummaryStories.WithAppliedAccountBalance} />
...
Total. The Subtotal row (extended prices before any discounts) appears only when a cart-level
discount is present:
