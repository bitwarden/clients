...
/**
 * Generates shared Storybook metadata for a policy's modal (non-drawer) story. Renders the same
 * dialog component as {@link policyDrawerMeta} but with `DialogRef.isDrawer = false`, so stories
 * can verify that the flag-off (modal) experience doesn't accidentally adopt v2/drawer styling.
 *
 * Usage is identical to {@link policyDrawerMeta} — set `title` as a literal string on the
 * default export after spreading, e.g.:
 * ```ts
 * export default {
 *   ...policyModalMeta(new MyPolicy()),
 *   title: "Admin Console/Organizations/Policies/My Policy/Modal (flag off)",
 * } satisfies Meta<PolicyDialogStoryArgs>;
 * ```
 */
