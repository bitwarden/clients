...
describe("InactiveTwoFactorReportComponent (organization)", () => {
  ...
  beforeEach(() => {
    ...
    component = TestBed.runInInjectionContext(
      () =>
        new InactiveTwoFactorReportComponent(
          cipherService,
          mock<DialogService>(),
          route,
          logService,
    ...
    // Pre-populate the 2FA directory so load2fa() short-circuits and does not call fetch().
  ...
  it("delegates context loading to OrgReportContextService and logs a successful initialization", async () => {
    ...
    expect(orgReportContext.load).toHaveBeenCalledWith(
      "org1",
      expect.stringContaining("[InactiveTwoFactorReport] [Enterprise]"),
    );
