...
@Component({
  ...
  template: `
    <popup-page>
      <popup-header slot="header" pageTitle="RAT SDK Test"></popup-header>

      <div class="tw-p-4 tw-space-y-3 tw-text-sm">
        <!-- Status -->
        <bit-callout [type]="statusType()">{{ statusMessage() }}</bit-callout>

        <!-- Init -->
        <div class="tw-space-y-2">
          <h3 class="tw-font-bold tw-text-main tw-mb-1">1. Initialize</h3>
          <input
            class="tw-w-full tw-p-2 tw-border tw-border-solid tw-border-secondary-300 tw-rounded tw-bg-background tw-text-main tw-text-xs tw-font-mono"
            [value]="proxyUrl()"
        ...
        <!-- Connect -->
        <div class="tw-space-y-2">
          <h3 class="tw-font-bold tw-text-main tw-mb-1">2. Connect</h3>
        ...
        @if (rendezvousCode()) {
          ...
          >
            ...
            <div class="tw-text-2xl tw-font-mono tw-font-bold tw-tracking-[0.3em] tw-text-main">
              {{ rendezvousCode() }}
            </div>
          </div>
        }
        @if (pskToken()) {
          ...
          >
            ...
            <div class="tw-text-xs tw-font-mono tw-text-main tw-break-all">{{ pskToken() }}</div>
          </div>
        }
        ...
        @if (pendingFingerprint()) {
          <div
            class="tw-bg-background tw-border tw-border-solid tw-border-warning-600 tw-rounded tw-p-3 tw-text-center"
          >
