...
| Content               | Spacing on either side |
| --------------------- | ---------------------- |
| Plain text            | 8px                    |
| Link                  | 8px                    |
| Badge / chip          | 4px                    |
...
plain text gets 8px, while a badge beside an icon button gets 4px. Wrapped content is measured by
what it contains, no matter how many components deep, so an icon button inside `<bit-item-action>`
or inside an app-level component is still treated as an icon button. Spacing a wrapper applies to
its own contents is ignored inside the slot.
