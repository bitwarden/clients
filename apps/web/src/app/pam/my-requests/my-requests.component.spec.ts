...
function makeResponse(fixture: ResponseFixture): LeaseRequestResponse {
...
describe("MyRequestsComponent", () => {
  ...
  beforeEach(async () => {
    pamApi = {
      ...
      patchLeaseRequest: jest.fn(),
      cancelLeaseRequest: jest.fn(),
      requestLeaseExtension: jest.fn(),
      decideLeaseRequest: jest.fn(),
      revokeLease: jest.fn(),
      setCollectionLeasingConfig: jest.fn(),
    ...
    await TestBed.configureTestingModule({
      imports: [MyRequestsComponent, NoopAnimationsModule],
      providers: [
        provideRouter([]),
  ...
  const create = async (
    responses: LeaseRequestResponse[],
  ): Promise<ComponentFixture<MyRequestsComponent>> => {
  ...
  it("cancels a pending request optimistically and calls the API", fakeAsync(() => {
    ...
    pamApi.cancelLeaseRequest.mockResolvedValue(undefined);
    ...
    expect(pamApi.cancelLeaseRequest).toHaveBeenCalledWith("p1");
  ...
  it("reverts the optimistic cancel when the API call fails", fakeAsync(() => {
    pamApi.listMyRequests.mockResolvedValue([makeResponse({ id: "p1", status: "pending" })]);
    pamApi.cancelLeaseRequest.mockRejectedValue(new Error("boom"));
...
describe("resolveResolverDisplayName", () => {
  const i18nMock = new I18nMockService({ pamResolverPolicy: "Policy" }) as unknown as I18nService;
  ...
  it("returns the policy label when there is no resolver user", () => {
    expect(
