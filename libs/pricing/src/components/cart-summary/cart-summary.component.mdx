...
- [Examples](#examples)
  ...
  - [With Percent-Off Discount](#with-percent-off-discount)
  - [With Amount-Off Discount](#with-amount-off-discount)
  - [With Custom Header](#with-custom-header)
  - [With Minimal Custom Header](#with-minimal-custom-header)
- [Features](#features)
...
| `discount`          | `Discount \| undefined`                                                  | **Optional.** Discount to apply (percent-off or amount-off)                                                 |
| `estimatedTax`      | `number`                                                                 | **Required.** Estimated tax amount                                                                          |
| `headerTemplate`    | `TemplateRef<{ total: number }> \| undefined`                            | **Optional.** Custom template for header content. Receives `total` as context for flexible header rendering |
...
export type Discount = {
  ...
  | {
      _tag: "percent-off";
      value: number; // Percentage (e.g., 30 for 30%)
    }
  | {
      _tag: "amount-off";
...
### With Percent-Off Discount

Show cart with a percentage-based discount:
...
### With Amount-Off Discount

Show cart with a fixed amount discount:
...
Show cart with a custom header template. The custom header receives the computed `total` value as
context:

<Canvas of={CartSummaryStories.WithCustomHeader} />
...
### With Minimal Custom Header

Show cart with a minimal custom header for simpler use cases:

<Canvas of={CartSummaryStories.WithMinimalCustomHeader} />

```html
<ng-template #customHeader let-total="total">
  <h3 style="margin: 0; font-size: 1.125rem;">Pay today: {{ total | currency: 'USD' }}</h3>
</ng-template>
...
- **Discount Support**: Handles both percentage-based and fixed-amount discounts with automatic
  calculation and display in success color styling
- **Customizable Header**: Supports custom header templates via the `headerTemplate` input, allowing
  parent components to provide context-specific header content (e.g., subscription status messages,
  renewal dates). Falls back to standard "Total: $X.XX USD / month" format when no template is
  provided.
