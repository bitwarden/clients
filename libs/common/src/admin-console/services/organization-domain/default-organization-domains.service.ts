...
import { ClaimedDomain, OrganizationId as SdkOrganizationId } from "@bitwarden/sdk-internal";
...
export class DefaultOrganizationDomainsService implements OrganizationDomainsService {
  ...
  async claimedDomains(userId: UserId, organizationId: OrganizationId): Promise<ClaimedDomain[]> {
      ...
      this.sdkService.userClient$(userId).pipe(
        concatMap(async (sdk) => {
          ...
          return await ref.value
            ...
            .get_claimed_domains(asUuid<SdkOrganizationId>(organizationId));
        }),
      ),
    );
  }
