...
export class SendPolicyService {
   ...
   *   - Flag ON  → checks `PolicyType.SendControls` with `data.disableSend`
   ...
   * Respects the `pm-31885-send-controls` feature flag:
   *   - Flag ON  → checks `PolicyType.SendControls` with `data.disableHideEmail`
    ...
    switchMap(([sendControlsEnabled, userId]) =>
      sendControlsEnabled
        ? this.policyService
            .policiesByType$(PolicyType.SendControls, userId)
            .pipe(
              map((policies) => policies?.some((p) => p.data?.disableHideEmail === true) ?? false),
            )
        : this.policyService
