...
- `app-actions`
  - One or more interactive elements on the right of the app bar. The bar is dark, so buttons
    projected here need `buttonType="side-nav"` to be legible against it.
- `start`
...
back into the back button or the `end` slot brings it into view rather than leaving the focus ring
clipped. It is only ever visually clipped, never removed from the accessibility tree. Under
