...
plain text gets 8px, while a badge beside an icon button gets 4px. Content wrapped in
`<bit-item-action>` is measured by what it contains, so a wrapped icon button is still treated as an
icon button.
