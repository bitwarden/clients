...
/**
 * The SDK only supports boolean feature flags at this time. Nullable, so callers holding a possibly-absent
 * server config (a fresh install, or a user who has not fetched one yet) can pass it straight through.
 */
export function toSdkFeatureFlags(config: ServerConfig | null): Map<string, boolean> {
  return new Map(
