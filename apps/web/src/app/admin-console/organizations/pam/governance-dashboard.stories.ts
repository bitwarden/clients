...
function makeRow(opts: {
  ...
  return new CollectionGovernanceRowResponse({
    ...
    Policy: opts.policy,
    RequireLeaseMemberCount: opts.requireLease ?? 0,
...
const busyRows = Array.from({ length: 12 }, (_, i) =>
  makeRow({
    id: `col-${i}`,
    name: `Collection ${i + 1}`,
    policy: i % 3 === 0 ? { Kind: "human_approval" } : null,
    requireLease: (i * 3) % 20,
    pending: i % 4,
    active: (i + 1) % 5,
...
/** N6: small org with 3 leasing-enabled collections exercising every policy kind. */
