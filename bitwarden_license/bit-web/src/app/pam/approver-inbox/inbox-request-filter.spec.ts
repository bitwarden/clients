...
function request(
  overrides: Partial<Pick<AccessRequestDetailsResponse, "leaseNotAfter" | "expiredAt">>,
): Pick<AccessRequestDetailsResponse, "leaseNotAfter" | "expiredAt"> {
  return { leaseNotAfter: null, expiredAt: null, ...overrides };
}
...
describe("isActionableInboxRequest", () => {
  ...
  it("keeps a request whose window has not yet elapsed", () => {
    expect(isActionableInboxRequest(request({ leaseNotAfter: "2026-06-15T13:00:00Z" }), NOW)).toBe(
      true,
    );
  });

  it("drops a request whose window has fully elapsed (timed out)", () => {
    expect(isActionableInboxRequest(request({ leaseNotAfter: "2026-06-15T11:00:00Z" }), NOW)).toBe(
      false,
    );
  });

  it("drops a request at the exact window boundary", () => {
    expect(isActionableInboxRequest(request({ leaseNotAfter: "2026-06-15T12:00:00Z" }), NOW)).toBe(
      false,
    );
  });
  ...
  it("drops a request the server marked as lapsed, even with a future window", () => {
    expect(
      isActionableInboxRequest(
        request({ leaseNotAfter: "2026-06-15T13:00:00Z", expiredAt: "2026-06-15T11:30:00Z" }),
        NOW,
      ),
    ).toBe(false);
  });
