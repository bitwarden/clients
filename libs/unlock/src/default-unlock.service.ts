...
export class DefaultUnlockService implements UnlockService {
  ...
  async unlockWithBiometrics(userId: UserId): Promise<void> {
    ...
    const startTime = performance.now();
    await this.unlockWithDecryptedKey(userId, userKey, UnlockSource.Manual);
  ...
  async unlockWithSharedUnlock(userId: UserId, userKey: SymmetricCryptoKey): Promise<void> {
    const startTime = performance.now();
    await this.unlockWithDecryptedKey(userId, userKey, UnlockSource.SharedUnlock);
    this.logService.measure(startTime, "Unlock", "DefaultUnlockService", "unlockWithSharedUnlock");
  }
  ...
  async unlockWithAutoUnlockKey(userId: UserId): Promise<boolean> {
    ...
    const startTime = performance.now();
    await this.unlockWithDecryptedKey(userId, userKey, UnlockSource.NeverLock);
    this.logService.measure(startTime, "Unlock", "DefaultUnlockService", "unlockWithAutoUnlockKey");
    return true;
  }

  private async unlockWithDecryptedKey(
    userId: UserId,
