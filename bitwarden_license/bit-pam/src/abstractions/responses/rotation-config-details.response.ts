 ...
 *
 * Mirrors the server's `PamRotationAttemptResponseModel`
...
export class RotationAttemptResponse extends BaseResponse {
  ...
  /** ISO-8601 timestamp when the daemon started executing this attempt — wire `CreationDate`. */
  startedAt: string;
  /**
   * ISO-8601 timestamp when the attempt completed, or null if still executing — wire `ResolvedDate`.
   */
  endedAt: string | null;
  ...
  constructor(response: unknown) {
    ...
    this.startedAt = this.getResponseProperty("CreationDate");
    this.endedAt = this.getResponseProperty("ResolvedDate") ?? null;
  }
}
 ...
 *
 * Mirrors the server's `PamRotationJobResponseModel`
 * (bitwarden/server `Bit.Services.Pam.Rotation.Api.Models.Response`).
...
export class RotationJobResponse extends BaseResponse {
  ...
  /**
   * ISO-8601 timestamp when the job was created (dispatched to the queue) — wire `CreationDate`.
   */
  createdAt: string;
  /**
  ...
  constructor(response: unknown) {
    ...
    this.status = this.getResponseProperty("Status");
    this.createdAt = this.getResponseProperty("CreationDate");
 ...
 *
 * Mirrors the server's `PamRotationConfigDetailResponseModel`, which extends the list model:
 * the config's own fields arrive flattened onto this same object (hence `super(response)`),
 * alongside `Jobs`. Jobs are newest first, each carrying its attempts oldest first, and are
 * parsed as nested {@link RotationJobResponse} BaseResponses. Wire property names are PascalCase.
 */
