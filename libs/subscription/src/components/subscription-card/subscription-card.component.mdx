...
| --------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| `callToActionClicked` | `PlanCardAction` (`"contact-support" \| "update-payment-method" \| "upgrade-plan" \| "reinstate-subscription"`) | Emitted when a call-to-action button is clicked in a status callout |
...
| `active` + cancelAt   | warning       | warning      | Active subscription pending cancellation, shows date with reinstate option   |
| `trialing`            | success       | info (opt.)  | Trial period with next charge date                                           |
| `trialing` + cancelAt | warning       | warning      | Trial subscription pending cancellation, shows date with reinstate option    |
| `past_due`            | warning       | warning      | Payment failed, shows grace period and suspension date                       |
| `past_due` + cancelAt | warning       | warning      | Past due subscription pending cancellation, shows date with reinstate option |
| `canceled`            | danger        | none         | Subscription canceled, shows cancellation date                               |
| `unpaid`              | danger        | danger       | Suspended due to non-payment                                                 |
| `incomplete`          | warning       | warning      | Payment incomplete, shows update payment and contact support options         |
| `incomplete_expired`  | danger        | danger       | Incomplete payment expired, shows contact support option                     |
...
Payment setup not completed, shows options to update payment or contact support:
...
Trial subscription scheduled to be canceled at end of trial with option to reinstate:
...
Active subscription scheduled to be canceled at end of billing period with option to reinstate:
