...
important when adding `bit-link` or `bit-button` to the content area sincce the callout backgournd
is colored. Currently only the `info` and `secondary` callouts are WCAG compliant for the `primary`
styling of these elements. The `secondary` `bit-link` styling may be used with the remainin
variants.
