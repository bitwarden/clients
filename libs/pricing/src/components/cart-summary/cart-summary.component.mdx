...
export type CartItem = {
  ...
  discounts?: CartDiscount[]; // Optional item-level discounts, rendered as rows beneath the line
...
- **Discount Support**: Displays both cart-level and item-level discounts. Cart-level discounts
  appear in a dedicated section, while each line item's `discounts` render as rows inline under the
  line item they apply to. Supports both percentage-based and fixed-amount discounts with green
  success styling
