...
import { SdkUnlockRequest } from "@bitwarden/common/platform/abstractions/sdk/sdk.service";
import {
  CommandDefinition,
  MessageListener,
  MessageSender,
...
type SdkPush =
  // `SdkUnlockRequest` is already JSON-safe. The request always carries a `decryptedKey` method here,
  // see the `unlock` override for why.
  | { kind: "unlock"; userId: UserId; request: SdkUnlockRequest }
...
export class BrowserSdkService extends DefaultSdkService {
  ...
  override async unlock(userId: UserId, request: SdkUnlockRequest): Promise<UserKey | null> {
