---
name: interact-client-lock
description: >
  Control lock and unlock behavior in the running Bitwarden app. Covers locking the vault and
  unlocking via biometrics (automation driver), PIN, or master password. Credentials are read from
  .debug/credentials.txt.
---

# Interact Client — Lock

Invoke this skill when you need to lock or unlock the vault in a running Bitwarden client. This
skill assumes the `interact-client` skill is already loaded and a DevTools session is active.
...
After locking, `wait_for` the lock screen to confirm the transition:

```js
// wait for the PIN input or biometric prompt to appear
```

Take a screenshot to confirm the vault is locked before proceeding.
...
launched with `USE_AUTOMATION_BIOMETRICS=1`.
...
- `interact-client` skill: MCP tool conventions, automation driver setup, DevTools connection
...
- `libs/common/src/platform/services/automation-driver.service.ts`: automation driver
