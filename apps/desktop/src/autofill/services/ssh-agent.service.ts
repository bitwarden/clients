  ...
  private async initListeners() {
    ...
    this.messageListener
      ...
      .pipe(
        ...
        //
        // Required even though the list-keys flow already prompts for unlock: keys are retained
        // in the agent's keystore across vault lock, so the agent can serve a sign request while
        // the vault is locked without ever firing the list callback.
    ...
    // The agent keystore itself is handled by the reactive block below.
  ...
  private async needsAuthorization(
    ...
    switch (promptType) {
      ...
      case SshAgentPromptType.RememberUntilLock: {
        const key = isForwarded ? hostFingerprint : SshAgentService.LOCAL_HOST_KEY;
        // hostFingerprint is optional on the request, so it may be absent; re-prompt rather
        // than caching an approval under a key we cannot distinguish between hosts.
        if (!key) {
          return true;
        }
