...
///
/// Pure: everything is passed in, and the caller writes the result. generate-entitlements.mts is
/// the shell that touches the filesystem.
...
// Re-exported so a caller composing entitlements does not have to reach for two modules, while
// the identifier itself is still spelled once, in channel.js, where after-pack.js can reach it.
export { TEAM_ID };
