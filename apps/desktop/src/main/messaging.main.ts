...
export class MessagingMain {
  ...
  private addOpenAtLogin() {
    console.log("addOpenAtLogin");
    if (process.platform === "linux") {
      if (isFlatpak()) {
        autostart.setAutostart(true, [AUTOSTART_FLAG]).catch((e) => {});
      } else if (isSnapStore()) {
        console.log("Adding snap store autostart");
  ...
  private updateSnapAutostartExec() {
    console.log("updateSnapAutostartExec");
    ...
    if (!snapUserData) {
      console.log("SNAP_USER_DATA env var not found, cannot set autostart flag on snap .desktop file");
      return;
    }

    console.log(`SNAP_USER_DATA found at ${snapUserData}, looking for .desktop file to set autostart flag on`);
    const autostartDir = path.join(snapUserData, ".config", "autostart");
    if (!fs.existsSync(autostartDir)) {
      console.log("Autostart directory not found");
      return;
    }
    console.log("Autostart directory found, looking for .desktop files");

    const desktopFiles = fs
      .readdirSync(autostartDir)
      .filter((fileName) => fileName.endsWith(".desktop"));

    console.log(`Found .desktop files: ${desktopFiles.join(", ")}`);
    for (const desktopFileName of desktopFiles) {
      ...
      console.log(`Updating .desktop file at ${desktopFilePath} to set autostart flag`);
      const updatedData = this.setAutostartFlagOnDesktopFile(existingData);

      console.log(`Updated .desktop file data:\n${updatedData}`);
      if (existingData !== updatedData) {
        console.log("Autostart flag not found in .desktop file, updating file");
        fs.writeFileSync(desktopFilePath, updatedData);
      } else {
        console.log("Autostart flag already set in .desktop file, no update needed");
      }
    }
  }
