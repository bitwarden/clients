...
export class CipherViewLikeUtils {
  ...
  static hasCopyableValue = (cipher: CipherViewLike, copyField: string): boolean => {
    ...
    if (this.isCipherListView(cipher)) {
      const type = this.getType(cipher);

      // Cross-check against the decrypted data the list views do expose. The SDK derives
      // `copyableFields` from whether a property is set, so a field stored as an empty string is
      // reported as copyable even though there is nothing to copy.
      if (type === CipherType.Login && copyField === "username") {
        return !!this.getLogin(cipher)?.username;
      }

      if (type === CipherType.BankAccount && copyField === "accountNumber") {
        return !!this.getBankAccount(cipher)?.accountNumber;
      }

      let _copyField = copyField;

      if (_copyField === "username" && type === CipherType.Identity) {
