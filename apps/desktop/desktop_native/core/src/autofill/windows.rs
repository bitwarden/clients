...
use win_webauthn::{
    plugin::{Clsid, PluginCredentialDetails, PluginUserVerificationRequest, WebAuthnPlugin},
    CredentialId, UserId,
};
...
fn handle_user_verification_request(
    ...
    let (buf, operation_request_hash) = request.transaction_context.split_at(16);
    let guid_u128 = buf
        .try_into()
